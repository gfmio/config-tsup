...
export type RawMerge<T extends Array<Partial<Options>>> = T extends [] ? Record<never, never> : T extends [infer U] ? U : T extends [infer U, ...infer R] ?  U extends Partial<Options> ? R extends Array<Partial<Options>> ? Merge2<U, Merge<R>> : never : never : never;
export type Merge<T extends Array<Partial<Options>>> = Simplify<RawMerge<T>>;
